/* global jQuery, N7_GOLF_CLUB_STORAGE */

jQuery( document ).on( 'action.ready_n7_golf_club', function() {
	...
	$document.on( 'action.resize_n7_golf_club', function() {
		_list_products_height = $list_products.length > 0 ? $list_products.height() : 0;
	} );
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.n7_golf_club_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		n7_golf_club_set_cookie( 'n7_golf_club_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// On first run
	n7_golf_club_woocommerce_add_inc_dec();
	...
	// Add buttons inc/dec
	function n7_golf_club_woocommerce_add_inc_dec() {
			...
			if ( $self.find('>input[name="quantity"][type="hidden"]').length === 0 ) {
				...
				$self
					.on( 'click', '>span', function(e) {
						n7_golf_club_woocomerce_inc_dec_click( jQuery( this ) );
						e.preventDefault();
						return false;
					} );
			} else {
	...
	function n7_golf_club_woocomerce_inc_dec_click(button) {
			...
			.on( 'click', function(e) {
				...
				jQuery.get(	link ).done(
					function(response) {
						...
						n7_golf_club_import_inline_styles( response );
						// Get tags 'link' from response and add its to the 'head'
						n7_golf_club_import_tags_link( response );
						...
						if ( posts_container.length > 0 ) {
							...
							N7_GOLF_CLUB_STORAGE['init_all_mediaelements'] = true;
							$document
	...
	$document.on( 'action.scroll_n7_golf_club', function(e) {
		...
		if ( $list_products.length > 0 && $list_products.offset().top + _list_products_height < n7_golf_club_window_scroll_top() + n7_golf_club_window_height() * 1.5 ) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		n7_golf_club_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					n7_golf_club_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					n7_golf_club_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		n7_golf_club_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			n7_golf_club_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function n7_golf_club_woocommerce_update_cart(status){
		setTimeout(
			function() {
				var w = window.innerWidth;
				if (w == undefined) {
					w = n7_golf_club_window_width() + ( n7_golf_club_window_height() < $document.height() || n7_golf_club_window_scroll_top() > 0 ? 16 : 0 );
				}

				var tbl = table_cart;

				if ( w < N7_GOLF_CLUB_STORAGE['mobile_layout_width'] ) {
				...
				if ( w >= N7_GOLF_CLUB_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
